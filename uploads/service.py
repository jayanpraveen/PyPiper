...
def get_file_info():
    ui = str(Upload.objects.all().first())
    path = glob.glob(f'media/videos/{ui}/*')[0]
    file_name, file_ext = os.path.splitext(path)
    file_name = os.path.basename(file_name)
    file_mime = mimetypes.guess_type(path)[0]

    file_info = namedtuple('file_info', ['name', 'ext', 'in_file', 'MIME', 'ui'])
    return file_info(file_name, file_ext, file_name+file_ext, file_mime, ui)
...
def video_to_audio():
    ...
    input_file = os.path.join(settings.MEDIA_ROOT, "videos", media.ui, media.in_file)
...
def remove_input_file(input_file):
    media = get_file_info()
    try:
        Upload.objects.get(video=f'videos/{media.ui}/{media.in_file}').delete()
